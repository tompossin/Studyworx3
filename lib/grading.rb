module Grading
  ...
  def grade_average(school_id,participant_id)
    results = Assignment.find_by_sql(["
      ...
      WHERE assignments.school_id=? and grades.participant_id=?
      GROUP BY grades.participant_id",school_id,participant_id])
